  		...
  		<script>
			...
			function calculate() {
				...
				fetch('/calculate', {
					...
					body: JSON.stringify({ myexpression: expression })
					})
			...
			function clearDisplay() {
  			...
  			document.addEventListener('keydown', function(event) {
  			  	const key = event.key;
			  	// Regular expression to match numeric keys
  			  	const allowedKeys = /^[0-9+\-*/()^!.]$/; 
  			  	if (allowedKeys.test(key)) {
  			    	// If the pressed key is a number, append it to the display
  			    	appendToDisplay(key);
  				}
  			  	const calculateExp = /^=$/;
  			  	if (calculateExp.test(key)){
  			  		calculate();
  			  	}
